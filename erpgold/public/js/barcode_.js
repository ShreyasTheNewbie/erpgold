...
erpnext.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		this.frm = opts.frm;
		frappe.flags.trigger_from_barcode_scanner3
		...
		this.custom_purity_percentage_field = opts.custom_purity_percentage__field || "custom_purity_percentage_";
	...
	update_table(data) {
		return new Promise(resolve => {
			...
			const { item_code, barcode, batch_no, serial_no, uom , custom_metal_type , custom_purity, custom_purity_percentage_} = data;

			let row = this.get_row_to_modify_on_scan(item_code, batch_no, uom, barcode , custom_metal_type , custom_purity ,custom_purity_percentage);
			...
			frappe.run_serially([
				...
				() => this.set_item(row, item_code, barcode, batch_no, serial_no, custom_metal_type , custom_purity, custom_purity_percentage_).then(qty => {
				...
				() => this.set_custom_purity_percentage(row, custom_purity_percentage),
				() => this.clean_up(),
	...
	set_item(row, item_code, barcode, batch_no, serial_no , custom_metal_type, custom_purity ,custom_purity_percentage) {
		return new Promise(resolve => {
			const increment = async (value = 1) => {
				...
				item_data[this.custom_purity_percentage_field] = custom_purity_percentage;
			...
			if (this.prompt_qty) {
				...
				this.prepare_item_for_scan(row, item_code, barcode, batch_no, serial_no , custom_metal_type, custom_purity ,custom_purity_percentage);
			} else {
	...
	set_custom_purity_percentage(row, custom_purity_percentage) {
		return new Promise(resolve => {
			if (custom_purity_percentage && frappe.meta.has_field(row.doctype, this.custom_purity_percentage_field)) {
				frappe.model.set_value(row.doctype, row.name, this.custom_purity_percentage_field, custom_purity_pecenatage)
					.then(() => resolve());
			} else {
				resolve();
			}
